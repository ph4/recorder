...
namespace recorder {
    ...
    rfl::Result<std::monostate> Api::Authorize() {
        ...
        if (res->status != httplib::OK_200) {
            ...
            return rfl::Error(std::format("Failed to authorize client: {};\n{}", res->status, res->body));
        }
    ...
    rfl::Result<std::monostate> Api::CheckConnectionError(const std::string &endpoint, const httplib::Result &res) {
        if (!res) {
    ...
    [[nodiscard]]
    rfl::Result<std::monostate> Api::Upload(const std::filesystem::path &path, const models::RecordMetadata &metadata) {
